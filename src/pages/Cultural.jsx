      ...
      <main className="max-w-6xl mx-auto px-4 pt-36 pb-12">
        ...
        <header className="text-center mb-8">
          <h1 className="text-4xl font-extrabold text-brownDark flex items-center justify-center gap-3">
            <Sparkles className="w-8 h-8" /> Cultural Events — Epistemico'26
          </h1>
          <p className="mt-2 text-brownMid max-w-2xl mx-auto">Explore cultural performances and register to participate.</p>
        </header>

        <div className="bg-gradient-to-r from-[#a9572f] to-[#8b4524] rounded-xl p-6 mb-8 text-white shadow-lg">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
              <h2 className="text-2xl font-bold">Perform, Create, Celebrate</h2>
              <p className="opacity-90">Pick a cultural event and join the stage.</p>
            </div>
            <button
          ...
          {culturalEvents.map((event) => (
            <div key={event.id} className="bg-white rounded-xl overflow-hidden shadow-lg flex flex-col">
              ...
              <div className="p-4 border-t border-gray-100 flex gap-2">
                <button onClick={() => navigate(`/cultural/${event.id}`)} className="flex-1 py-2 bg-[#a9572f] text-white rounded-full font-semibold">Explore Now</button>
